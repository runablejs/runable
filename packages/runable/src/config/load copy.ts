...
export async function loadAndCacheConfig({
  ...
  const promise = (async () => {
    ...
    // (i.e. `defineModule`'d config); the rest is a plain `RunableConfig` and
    ...
    let rConfig = resolveConfig(config);
    rConfig._name = name;
    rConfig._configFile = _configFile;
    rConfig._isRunableModule = config._isRunableModule;
 ...
 *   then `dist/` is appended, since Runable modules are expected to publish their
...
function getModuleDir(name: string, parent: ResolvedConfig): string {
  ...
  if (!cwd) {
    try {
      const packageJsonPath = resolvePackageDir(name /**import.meta.dirname */);
