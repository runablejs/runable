...
import { useAllConfigs, useConfig } from "./load.js";
 ...
 * of `RunableConfig` that types each loaded module's options key precisely,
 * instead of relying on `RunableConfig`'s `[key: string]: unknown` escape
 * hatch.
 *
 * Each entry references the module config's TypeScript source or adjacent
 * declaration file and lets TypeScript infer `OptionsT` from its default
 * `ModuleDefinition<OptionsT>` export. This preserves types for compiled npm
 * modules because the generic survives in `runable.config.d.ts`, even though it
 * has been erased from `runable.config.js`.
 *
 * The Babel extraction below remains as a compatibility fallback for module
 * configs that do not expose a resolvable TypeScript source or declaration.
 *
...
export async function generateModulesOptionsDts(): Promise<string> {
  ...
  for (const moduleConfig of modules) {
    const configKey = moduleConfig._configKey ?? moduleConfig._name;
    if (entries.has(configKey)) continue; // first module to claim a key wins

    const moduleType = resolveModuleType(moduleConfig, outFile);
    if (moduleType) {
      entries.set(
        configKey,
        `ExtractRunableModuleOptions<(typeof import("${moduleType}"))["default"]>`,
      );
      continue;
    }
    ...
    if (!extracted?.optionsTypeText) {
      console.warn(
        `[Runable] No TypeScript declaration was found for module "${moduleConfig._name}". Its options will use Record<string, unknown>. Publish a declaration alongside "${moduleConfig._configFile ?? "runable.config.js"}" to preserve the module options type.`,
      );
...
function resolveModuleType(
  ...
  const isTypeScriptSource = /\.[cm]?tsx?$/.test(configFile);
  if (!isTypeScriptSource) {
    const declarationCandidates = configFile.endsWith(".mjs")
      ? [
          configFile.replace(/\.mjs$/, ".d.mts"),
          configFile.replace(/\.mjs$/, ".d.ts"),
        ]
      : configFile.endsWith(".cjs")
        ? [
            configFile.replace(/\.cjs$/, ".d.cts"),
            configFile.replace(/\.cjs$/, ".d.ts"),
          ]
        : [configFile.replace(/\.js$/, ".d.ts")];

    if (!declarationCandidates.some(existsSync)) return;
  }

  let modulePath = normalizeDir(relative(dirname(outFile), configFile));
...
function writeDts(
  ...
  const configTypePath = normalizeDir(
    relative(dirname(outFile), join(import.meta.dirname, "./types.ts")),
  );

  const moduleDefinitionImport = `import type { ModuleDefinition as RunableModuleDefinition } from "${configTypePath}";`;

  const content = `// Auto-generated by Runable — do not edit by hand.
// Regenerated on every loadConfig() call from each module's \`defineModule<OptionsT>\` call.

${moduleDefinitionImport}
${imports ? `${imports}\n` : ""}
type ExtractRunableModuleOptions<T> = T extends RunableModuleDefinition<infer Options>
  ? Options
  : Record<string, unknown>;

${localDeclarations ? `${localDeclarations}\n\n` : ""}
declare module "${configTypePath}" {
