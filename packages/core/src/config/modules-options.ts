 ...
 * modules because the generic survives in `syora.config.d.ts`, even though it
 * has been erased from `syora.config.js`.
 *
...
export async function generateModulesOptionsDts(): Promise<string> {
  ...
  for (const moduleConfig of modules) {
    ...
    if (moduleType) {
      entries.set(
        configKey,
        `ExtractSyoraModuleOptions<(typeof import("${moduleType}"))["default"]>`,
      );
      continue;
    }
    ...
    if (!extracted?.optionsTypeText) {
      console.warn(
        `[Syora] No TypeScript declaration was found for module "${moduleConfig._name}". Its options will use Record<string, unknown>. Publish a declaration alongside "${moduleConfig._configFile ?? "syora.config.js"}" to preserve the module options type.`,
      );
...
function writeDts(
  ...
  const moduleDefinitionImport = `import type { ModuleDefinition as SyoraModuleDefinition } from "${configTypePath}";`;
...
${imports ? `${imports}\n` : ""}
type ExtractSyoraModuleOptions<T> = T extends SyoraModuleDefinition<infer Options>
  ? Options
