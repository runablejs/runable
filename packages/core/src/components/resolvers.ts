...
/**
 * Collects every resolver declared on `options.dirs` object entries,
 * alongside the top-level `options.resolvers`, into a single flat list.
 * Resolution stays non-directory-scoped: top-level resolvers are tried
 * first, then per-entry ones in declaration order.
 */
export function collectResolvers(
  options: ResolvedOptions,
): NonNullable<ResolvedOptions["resolvers"]> {
  const entries = toArray(options.dirs, []);
  const fromEntries = entries.flatMap((entry) =>
    typeof entry === "string" ? [] : (entry.resolvers ?? []),
  );
  return [...(options.resolvers ?? []), ...fromEntries];
}
...
function normalizeResult(
  ...
  if (typeof result === "string") return { name: "default", from: result };
  return result;
}
 ...
 */
export function createResolverEngine(
  resolvers: (ComponentResolver | ComponentResolver[])[] = [],
): ResolveFn {
