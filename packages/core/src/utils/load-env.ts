...
// Kept in sync with the `syora:runtime` Vite plugin.
const RELEVANT_PREFIX = /^(SYO_|SYORA_|VITE_)/;
const PUBLIC_PREFIX = /^(SYO_PUBLIC_|SYORA_PUBLIC_|VITE_PUBLIC_)/;
 ...
 * keeps only the `SYO_`/`SYORA_`/`VITE_`-prefixed variables.
 *
 * Returns both:
 * - `raw`: original prefixed names -> value
 * - `runtime`: same split used by the `syora:runtime` Vite plugin
