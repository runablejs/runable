...
> = Omit<A, "ui"> & {
  ui: U extends "ui.prose"
    ? (A extends { ui: infer UI }
        ? Omit<UI, "prose">
        : Record<string, never>) & {
        prose?: (A extends { ui: { prose?: infer P } }
          ? Omit<NonNullable<P>, K>
          : Record<string, never>) & {
          [k in K]?: Partial<T>;
        };
      }
 ...
 * @template A The base SyoraConfig type from `@syora/core`.
 * @template K The key identifying the component (e.g., 'badge').
 * @template U The top-level key in SyoraConfig
 */
