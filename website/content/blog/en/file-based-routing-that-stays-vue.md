...
Runable removes that duplicate structure. Files inside `app/pages/` become routes, while Vue Router remains the routing engine underneath.
...
A nested URL does not always need a nested interface. When it does, place `RunablePage` in the parent page:
...
The child route renders where `RunablePage` appears. This keeps the filesystem responsible for route hierarchy and the component responsible for visual hierarchy.
...
Runable generates alphanumeric PascalCase names for routes. A file such as `projects/[id].vue` receives a name such as `ProjectsId`. If the public name is part of your application API, set it explicitly:
...
Use `RunableLink` for links rendered in templates:

```vue
<RunableLink :to="{ name: 'ProjectDetails', params: { id: project.id } }">
  {{ project.name }}
</RunableLink>
```
...
For guards, history inspection, or another advanced Vue Router feature, call `useRouter()` directly. Runable adds a productive default; it does not replace the underlying APIs.
