...
`.app/tsconfig.app.json` is generated by `syora prepare`. It already contains `app/` sources, Syora declarations, aliases, and Vue-specific options. You do not need a root `tsconfig.app.json` unless you want project-specific settings.
...
Run `syora prepare` first, especially after a new installation or after deleting `.app/`.
