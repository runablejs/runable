...
`.app/tsconfig.app.json` est généré par `runable prepare`. Il contient déjà les sources de `app/`, les déclarations Runable, les alias et les options adaptées à Vue. Vous n'avez donc pas besoin d'un `tsconfig.app.json` à la racine tant que vous ne souhaitez pas ajouter de réglage propre au projet.
...
Exécutez `runable prepare` avant ce contrôle, notamment après une nouvelle installation ou après la suppression de `.app/`.
